...
let glassList = () =>{
    let content = ""
    dataGlasses.map((index)=>{
        content += `
        <div class="col-4">
            <img style="cursor: pointer;" onclick="glassInfo('${index.id}')" class="img-fluid" src="${index.src}" alt="">
        </div>   
        `
    })
...
let glassInfo = (id) => {
    dataGlasses.map((index) =>{
        if(index.id === id){
            let { virtualImg, brand, name, color, price, description } = index;
...
window.glassInfo = glassInfo;


let removeGlasses = (glassRemove) => {
    if (glassRemove) {
